...
| AI UX / Human-AI Interaction | Provisional L2 | L4 | CORE | Loading/success/failure, approval and uncertainty reasoning; P1 runtime streaming evidence plus P2 learner-verified pending approval, rejected, invalidated, Tool failure, successful retry / recovery, and retry invalidation after external state change, including user-visible states and side-effect boundaries. Broader AI UX and user validation remain unverified. | Medium | P3 capability UX | 2026-09-07 |
| Workflow Design | Provisional L2 | L4 | CORE | Deterministic/model/tool/validator responsibility and perturbation adaptation; P2 integrates fixed Workflow with local Agent autonomy, routing / chaining / parallelization / gates, and runtime-enforced approval / revalidation. Learner personally verified normal approval, reject, stale invalidation, Tool failure, successful retry / recovery, and retry invalidation after external state change in the deterministic micro lab. | Medium | P3 capability-boundary practice | 2026-09-07 |
...
| Eval | Not Assessed | L4 | CORE | P1 basic conceptual coverage: Rubric, Eval Set, Regression, Human Judge, Programmatic Judge, LLM Judge, Hard Gate, and Soft Eval. Codex-assisted mutation-testing demonstration verified a runtime Oracle catching build-pass / behavior-fail execution. P2 case reasoning adds trace comparison and execution-success versus decision-quality exposure; D1 / D2 add learner runtime verification of retry success and retry invalidation, but evidence remains insufficient for a formal capability level. | Low | P3 capability Eval practice, then P6 deepening | 2026-09-07 |
...
| Tool Calling | Confirmed L1 | L3 | CORE-SUPPORT | Understands tool/API, read/write, approval and permission boundary. P1 local runtime Tool Calling (Codex-assisted) covered Tool Schema validation, `tool_call_id` matching for multiple / parallel Tool Results, dependent serial Tool Calls, and invalid schema → runtime reject → no execution. P2 adds local deterministic tool-boundary transfer: frozen pending action, capability versus permission / approval, revalidation before execution, Tool failure, successful retry / recovery, and retry invalidation after external state change. Real Provider Tool Calling remains unverified. | Medium | Deferred real Provider Tool Calling when Provider API access is available; natural P3 capability-boundary practice | 2026-09-07 |
...
| Agent Design | Not Assessed | L3-L4 | CORE-SUPPORT | P2 conceptual / architectural transfer: fixed Workflow can contain local Agent autonomy; runtime action choice differs from fixed execution; public product-case reasoning covered sandbox, firewall, approval, merge-control, and tool-interface effects. No real Agent Runtime implementation evidence. | Low | Natural P3 capability-boundary practice; real Agent Runtime only when needed | 2026-09-07 |
